...
_, edr_ica = picker.apply(ica_edr.T, r_timestamps, sort_=True, crop_to_freq=.6)
_, edr_pca = picker.apply(pca_edr, r_timestamps, sort_=True, crop_to_freq=.6)

pl = CollectivePlotter(3, 2)

edr_domain = np.arange(r_timestamps[0], r_timestamps[-1], 1/SAMPLING_FREQ)

pl.set_col_values(0, edr_ica[:3])
pl.set_col_values(1, edr_pca[:3])
